...
class _Vbulletin5 extends \IPS\convert\Software
{
	...
	public function convertForumsForums()
	{
		...
		foreach( $this->fetch( 'node', 'nodeid', array( "node.nodeid<>2 AND ( ( node.parentid=? AND node.contenttypeid=? ) OR " . $this->db->in( 'nodeid', array_keys( $clubs ) ) . " )", $this->fetchType( 'Thread' ), $this->fetchType( 'Channel' ) ) ) AS $forum )
		{
			$self = $this;
