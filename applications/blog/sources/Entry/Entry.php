	...
	*/
	...
	public static function getItemsWithPermission( $where=array(), $order=NULL, $limit=10, $permissionKey='read', $includeHiddenItems=\IPS\Content\Hideable::FILTER_AUTOMATIC, $queryFlags=0, \IPS\Member $member=NULL, $joinContainer=FALSE, $joinComments=FALSE, $joinReviews=FALSE, $countOnly=FALSE, $joins=NULL, $skipPermission=FALSE, $joinTags=TRUE, $joinAuthor=TRUE, $joinLastCommenter=TRUE, $showMovedLinks=FALSE )
	{
		if ( in_array( $permissionKey, array( 'view', 'read' ) ) )
		{
			...
			if ( \IPS\Settings::i()->clubs )
			{
				...
				if ( $member->member_id )
	            {
					$where[] = array( '( blog_blogs.blog_club_id IS NULL OR ' . \IPS\Db::i()->in( 'blog_blogs.blog_club_id', $member->clubs() ) . ' OR core_clubs.type=? )', \IPS\Member\Club::TYPE_PUBLIC );
	            }
	            else
	            {
					$where[] = array( '( blog_blogs.blog_club_id IS NULL OR core_clubs.type=? )', \IPS\Member\Club::TYPE_PUBLIC );
	            }
			}
		}
	...
	public static function followWhere( &$joinContainer, &$joins )
	{
		...
		if ( \IPS\Settings::i()->clubs )
		{
			...
			if ( \IPS\Member::loggedIn()->member_id )
            {
				$where[] = array( '( blog_blogs.blog_club_id IS NULL OR ' . \IPS\Db::i()->in( 'blog_blogs.blog_club_id', \IPS\Member::loggedIn()->clubs() ) . ' OR core_clubs.type=? )', \IPS\Member\Club::TYPE_PUBLIC );
            }
            else
            {
				$where[] = array( '( blog_blogs.blog_club_id IS NULL OR core_clubs.type=? )', \IPS\Member\Club::TYPE_PUBLIC );
