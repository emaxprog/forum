...
class _view extends \IPS\Content\Controller
{
	...
	protected function manage()
	{
		...
		\IPS\Output::i()->jsonLd['download']	= array(
			...
			'name'			=> htmlspecialchars( $this->file->mapped('title'), ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ),
			...
			'softwareVersion'	=> htmlspecialchars( $this->file->version, ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ),
			'author'		=> array(
				'@type'		=> 'Person',
				'name'		=> htmlspecialchars( \IPS\Member::load( $this->file->submitter )->name, ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ),
				'url'		=> (string) \IPS\Member::load( $this->file->submitter )->url(),
				...
				foreach ( $values['screenshots'] as $_key => $file )
				{
					...
					else
					{
						...
						if ( $watermarked !== TRUE )
						{
							try
							{
								$existing = \IPS\Db::i()->select( '*', 'downloads_files_records', array( "record_location=? AND record_file_id=?", (string) $file, $this->file->id ) )->first();
							}
							catch( \UnderflowException $e ) {}
						}
