...
class _posting extends \IPS\Dispatcher\Controller
{
	...
	protected function _manageGeneral()
	{
		...
		$form->hiddenValues['rebuildImageProxy'] = \IPS\Request::i()->rebuildImageProxy ?: 0;
		...
		$form->add( new \IPS\Helpers\Form\Number( 'dellog_retention_period', \IPS\Settings::i()->dellog_retention_period, FALSE, array( 'unlimited' => 0, 'unlimitedLang' => 'immediately' ), NULL, NULL, \IPS\Member::loggedIn()->language()->addToStack('dellog_days_after_deleted') ) );
		...
		if ( $values = $form->values() )
		{
			...
			/* Run the image proxy rebuild tasks */
			if( $values['rebuildImageProxy'] )
			{
				/* Remove any existing rebuilds */
				\IPS\Db::i()->delete( 'core_queue', array( '`key`=? OR `key`=?', 'RebuildImageProxy', 'RebuildImageProxyNonContent' ) );

				/* Unset task datastore */
				unset( \IPS\Data\Store::i()->currentImageProxyRebuild );

				$enableDisable = $values['remote_image_proxy'] ? TRUE : FALSE;
				foreach ( \IPS\Content::routedClasses( FALSE, TRUE ) as $class )
				{
					try
					{
						\IPS\Task::queue( 'core', 'RebuildImageProxy', array( 'class' => $class, 'status' => $enableDisable ), 5 );
					}
				...
				foreach( \IPS\Application::allExtensions( 'core', 'EditorLocations', FALSE, NULL, NULL, TRUE, TRUE ) as $_key => $extension )
				{
					...
					{
						\IPS\Task::queue( 'core', 'RebuildImageProxyNonContent', array( 'extension' => $_key, 'status' => $enableDisable ), 5 );
					}
				}
			}
