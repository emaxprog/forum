...
class _promote extends \IPS\Dispatcher\Controller
{
	...
	protected function _manageTwitter()
	{
		...
		if ( ! $twitter->settings['consumer_key'] )
		{
	...
	protected function _manageFacebook()
	{
		...
		{
			/* Nope, so lets do that now... */
			return \IPS\Theme::i()->getTemplate( 'promote' )->warningNoFacebookUser();
		}
		...
		{
			/* Nope, so lets do that now... */
			return \IPS\Theme::i()->getTemplate( 'promote' )->warningNoFacebookPostToPagePermission( $response );
		}
		...
		if ( $account->settings['owner'] and $account->settings['page_name'] )
		{
			try
			{
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'promote_facebook_enabled', $account->enabled, FALSE, array( 'togglesOn' => array( 'promote_facebook_page', 'promote_facebook_groups', 'promote_facebook_members', 'promote_facebook_tags' ) ), NULL, NULL, NULL, 'promote_facebook_enabled' ) );
		
		if ( ! $account->settings['owner'] or ( \IPS\Member::loggedIn()->member_id == $account->settings['owner'] ) )
		{
			$form->add( new \IPS\Helpers\Form\Select( 'promote_facebook_page', $account->settings['page'], FALSE, array( 'options' => $options ), NULL, NULL, NULL, 'promote_facebook_page' ) );
		}
		...
		{ 
			...
			if ( $values['promote_facebook_page'] )
			{
				/* Get a non-expiring page token */
				$save['token'] = $facebook->exchangeToken( $pages[ $values['promote_facebook_page'] ][1] );
				$save['owner'] = \IPS\Member::loggedIn()->member_id;
				$save['page_name'] = $pages[ $values['promote_facebook_page'] ][0];
				$save['page'] = $values['promote_facebook_page'];
		...
		$table->parsers = array(
			...
			'response_promote_id' => function( $val, $row )
			{
				try
				{
					...
					return $promote->objectTitle . ' (' . $val . ')';
