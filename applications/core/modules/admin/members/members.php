...
class _members extends \IPS\Dispatcher\Controller
{
	...
	public function massManage()
	{
		...
		foreach ( array( 'joined', 'last_post', 'last_visit' ) as $k )
		{
			...
			if ( \IPS\Request::i()->$requestKey['start'] or \IPS\Request::i()->$requestKey['end'] )
			{
				$start = NULL;
				$end = NULL;
				if ( \IPS\Request::i()->$requestKey['start'] )
				{
					$start = new \IPS\DateTime( \IPS\Helpers\Form\Date::_convertDateFormat( \IPS\Request::i()->$requestKey['start'] ) );
					$start = $start->setTime( 0, 0, 0 );
				}
				if ( \IPS\Request::i()->$requestKey['end'] )
				{
					$end = new \IPS\DateTime( \IPS\Helpers\Form\Date::_convertDateFormat( \IPS\Request::i()->$requestKey['end'] ) );
