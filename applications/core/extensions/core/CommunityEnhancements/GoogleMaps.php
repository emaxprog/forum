...
class _GoogleMaps
{
	...
	public function toggle( $enabled )
	{
		if ( $enabled )
		{
			...
			{
				\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => 1 ), array( '( conf_key=? OR conf_key=? )', 'googlemaps', 'googleplacesautocomplete' ) );
		...
		{
			\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => 0 ), array( '( conf_key=? OR conf_key=? )', 'googlemaps', 'googleplacesautocomplete' ) );
			unset( \IPS\Data\Store::i()->settings );
		}
	}
}