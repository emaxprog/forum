...
class _ProfileFields
{
	...
	public static function formatFormValues( $values, &$member, &$form )
	{
		...
		if ( count( $profileFields ) )
		{
			try
			{
				\IPS\Db::i()->select( '*', 'core_pfields_content', array( "member_id=?", $member->member_id ) )->first();
				\IPS\Db::i()->update( 'core_pfields_content', $profileFields, array( "member_id=?", $member->member_id ) );
			}
			catch( \UnderflowException $e )
			{
				\IPS\Db::i()->insert( 'core_pfields_content', array_merge( array( 'member_id' => $member->member_id ), $profileFields ) );
			}
			...
			if ( $step->completion_act === 'profile_fields' AND ! $step->completed( $member ) )
			{
				$wizards[ $step->key ] = function( $data ) use ( $member, $step, $profileFields ) {
					...
					foreach( $step->subcompletion_act as $item )
					{
						...
						if ( !$field->admin_only )
						{
							$value = $profileFields['core_pfield_' . $id];
							if ( $field->multiple )
							{
