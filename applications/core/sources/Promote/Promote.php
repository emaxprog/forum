...
class _Promote extends \IPS\Patterns\ActiveRecord
{
	...
	public function get_objectDataCount()
	{
		...
		if ( $object instanceof \IPS\Content\Item )
		{
			if ( isset( $object::$databaseColumnMap['num_comments'] ) and isset( $object::$commentClass ) )
			{
	...
	public function send()
	{
		...
		foreach( $this->share_to as $service )
		{
			...
			{
				/* It failed, increment counter and set the next attempt for 10 minutes time */
				$this->failed++;
	...
	public static function objectContent( $object )
	{
		if ( $object instanceof \IPS\Content\Item )
		{
			...
			else if ( $object::$firstCommentRequired )
			{
				...
				return trim( preg_replace( "#(<br>){1,}#", "\n",  preg_replace( '#(<br>)\s+#', "\n", $firstComment->truncated() ) ) );
			}
		...
		{
			return trim( strip_tags( preg_replace( "#(<br>){1,}#", "\n",  preg_replace( '#(<br>)\s+#', "\n", $object->description ) ) ) );
		}
