	...
	protected static function _checkUrlFilters( $content )
	{
		...
		if ( $urls )
		{
			try
			{
				...
				$tags = $selector->query('//img | //a');
				$good = NULL;
				foreach( $tags AS $tag )
				{
					if ( ( $tag->hasAttribute( 'href' ) and !$tag->hasAttribute( 'data-mentionid' ) ) OR ( $tag->hasAttribute( 'src' ) AND !$tag->hasAttribute( 'data-emoticon' ) ) )
					{
						...
						foreach( $urls AS $url )
						{
							...
							if ( $url )
							{
								...
								else if ( \IPS\Settings::i()->ipb_url_filter_option == 'white' )
								{
									...
									if ( is_null( $good ) AND !preg_match( '/' . $url . '/i', str_replace( '<___base_url___>', 'http://' . parse_url( \IPS\Settings::i()->base_url, PHP_URL_HOST ), $urlToCheck ), $matches ) )
									{
										$good = FALSE;
										break 2;
									}
