...
class _view extends \IPS\Content\Controller
{
	...
	protected function manage()
	{
		...
		\IPS\Output::i()->jsonLd['gallery']	= array(
			...
			'name'			=> htmlspecialchars( $this->image->mapped('title'), ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ),
			...
			'author'		=> array(
				'@type'		=> 'Person',
				'name'		=> htmlspecialchars( \IPS\Member::load( $this->image->member_id )->name, ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ),
				'url'		=> (string) \IPS\Member::load( $this->image->member_id )->url(),
		...
		if( $this->image->media )
		{
			...
			\IPS\Output::i()->jsonLd['gallery']['contentSize'] = htmlspecialchars( $this->image->mapped('title'), ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE );
		}
		else
		{
			try
			{
				...
				\IPS\Output::i()->jsonLd['gallery']['image']		= array(
					...
					'caption'	=> htmlspecialchars( $this->image->mapped('title'), ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ),
	...
	protected function move()
	{
		try
		{
			...
			$form->add( new \IPS\Helpers\Form\Node( 'move_to_category', NULL, NULL, array( 'clubs' => true, 'class' => 'IPS\\gallery\\Category', 'permissionCheck' => 'add' ), function( $val ) {
