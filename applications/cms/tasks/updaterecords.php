...
class _updaterecords extends \IPS\Task
{
	...
	public function execute()
	{
		foreach( \IPS\cms\Databases::databases() as $database )
		{
			...
			{
				foreach( \IPS\Db::i()->select( '*', $recordClass::$databaseTable, array( 'record_expiry_date > 0 and record_expiry_date <= ? and record_approved=1', time() ),'primary_id_field ASC', 250, NULL, \IPS\Content\Hideable::FILTER_SHOW_HIDDEN ) as $row )
				{
