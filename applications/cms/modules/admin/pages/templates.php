...
class _templates extends \IPS\Dispatcher\Controller
{
	...
	public function import()
	{
		...
		if ( $values = $form->values() )
		{
			if ( $values['cms_templates_import'] )
			{
				...
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=cms&module=pages&controller=templates&do=importProcess' )->setQueryString( array( 'file' => $tempFile, 'key' => md5_file( $tempFile ) ) ) );
			}
			else
			{
	...
	public function importProcess()
	{
		...
		$prefix = date('m') . '_' . date('d') . '_' . date('y') . '_';
	...
	public function databaseTemplateGroupOptions()
	{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'cms_database_group_name', \IPS\cms\Templates::readableGroupName( \IPS\Request::i()->group ), NULL, array( 'regex' => '/^([A-Z_][A-Z0-9_\.\s]+?)$/i' ) ) );
		$form->addButton( "delete", "link", \IPS\Http\Url::internal( 'app=cms&module=pages&controller=templates&do=deleteTemplateGroup&group=' . \IPS\Request::i()->group . '&t_location=' . \IPS\Request::i()->t_location ), 'ipsButton ipsButton_negative', array( 'data-confirm' => 'true' ) );
		...
		if ( $values = $form->values() )
		{
			...
			if ( $new != \IPS\Request::i()->group )
			{
				...
				{
					\IPS\Db::i()->update( 'cms_databases', array( 'database_template_' . $field => $new ), array( 'database_template_' . $field . ' =?', \IPS\Request::i()->group ) );
				}
			}
