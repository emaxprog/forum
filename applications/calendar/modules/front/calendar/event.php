...
class _event extends \IPS\Content\Controller
{
	...
	protected function manage()
	{
		...
		\IPS\Output::i()->jsonLd['event']	= array(
			...
			'name'			=> htmlspecialchars( $this->event->mapped('title'), ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ),
		...
		if( $this->event->container()->allow_reviews )
		{
			...
			\IPS\Output::i()->jsonLd['event']['interactionStatistic'][]	= array(
				'@type'					=> 'InteractionCounter',
				'interactionType'		=> "http://schema.org/ReviewAction",
				'userInteractionCount'	=> $this->event->reviews
			);
		}

		if( $this->event->container()->allow_comments )
		{
			\IPS\Output::i()->jsonLd['event']['interactionStatistic'][] = array(
				'@type'					=> 'InteractionCounter',
				'interactionType'		=> "http://schema.org/CommentAction",
				'userInteractionCount'	=> $this->event->mapped('num_comments')
			);

			\IPS\Output::i()->jsonLd['event']['commentCount'] = $this->event->mapped('num_comments');
		}
		...
		if( $this->event->rsvp )
		{
			\IPS\Output::i()->jsonLd['event']['potentialAction'] = array(
				'@type'		=> 'RsvpAction',
				'target'	=> (string) $this->event->url( 'rsvp' )->setQueryString( 'action', '{rsvpResponse}' ),
				'rsvpResponse-input' => "required name=rsvpResponse"
			);
			...
			if( count( $attendees[1] ) )
			{
				...
				foreach( $attendees[1] as $attendee )
				{
					\IPS\Output::i()->jsonLd['event']['attendee'][] = array(
						'@type'		=> 'Person',
						'name'		=> htmlspecialchars( $attendee->name, ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE )
					);
				}
			}
		}
