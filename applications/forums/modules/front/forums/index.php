...
class _index extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		{
			$where = array( 'container' => array( array( 'forums_forums.password IS NULL AND forums_forums.club_id IS NULL' ) ) );
			$forumIds = array();
			$parentIds = array();
			$map = array();
			$ids = array();
			...
			if ( count( $ids ) )
			{
				foreach( $ids as $id )
				{
					try
					{
						...
						{
							/* This is a root category, so fetch all children */
							$this->getParentIds( $forum, $parentIds );
							$forumIds[] = $forum->id;
						}
						else
						{
				...
				/* Now remove all children that are in the same category */
				foreach( $map as $parent => $children )
				{
					if ( in_array( $parent, $parentIds ) )
					{
						$forumIds = array_diff( $forumIds, $children );
					}
				}
				...
				if ( count( $forumIds ) )
				{
					$or[] = \IPS\Db::i()->in( 'forums_forums.id', array_filter( $forumIds ) );
				}
				
				if ( count( $parentIds ) )
				{
					$or[] = \IPS\Db::i()->in( 'forums_forums.parent_id', array_filter( $parentIds ) );
				}
				
				if ( count( $or ) )
				{
					$where['container'][] = array( '(' . implode( ' OR ', $or ) . ')' );
				}
			}
			...
			{
				\IPS\Output::i()->sidebar['contextual'] = \IPS\Theme::i()->getTemplate( 'index' )->simplifiedViewSidebar( $forumIds, $parentIds );
			}
	 ...
	 *
	 * @param	\IPS\forums\Forum	$forum		Forum object
	 * @param	array				$parentIds	ParentIds to use
	 * @return void
	 */
	protected function getParentIds( $forum, &$parentIds )
	{
		if ( $forum->hasChildren() )
		{
			$parentIds[ $forum->id ] = $forum->id;
			
			foreach( $forum->children() as $child )
			{
				if ( $child->hasChildren() )
				{
					$parentIds[ $child->id ] = $child->id;
					
					$this->getParentIds( $child, $parentIds );
				}
			}
		}
	}
	...
	protected function setMethod()
	{
		...
		{
			\IPS\Db::i()->replace( 'forums_view_method', array( 'member_id' => \IPS\Member::loggedIn()->member_id, 'method' => $method ) );
		}
