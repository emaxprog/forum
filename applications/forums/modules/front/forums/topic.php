...
class _topic extends \IPS\Content\Controller
{
	...
	protected function manage()
	{
		...
		\IPS\Output::i()->jsonLd['topic']	= array(
			...
			'name'			=> htmlspecialchars( $topic->mapped('title'), ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ),
			'headline'		=> htmlspecialchars( $topic->mapped('title'), ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ),
			...
			'author'		=> array(
				'@type'		=> 'Person',
				'name'		=> htmlspecialchars( $topic->starter_name ?: \IPS\Member::load( $topic->starter_id )->name, ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ),
			...
			if( $topic->topic_answered_pid )
			{
				try
				{
					...
					\IPS\Output::i()->jsonLd['topic']['acceptedAnswer'] = array(
						...
						'author'	=> array(
							'@type'		=> 'Person',
							'name'		=> htmlspecialchars( $answer->author_name ?: \IPS\Member::load( $answer->author_id )->name, ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ),
							'url'		=> (string) \IPS\Member::load( $answer->author_id )->url(),
		...
		else
		{
			...
			foreach( $comments as $comment )
			{
				\IPS\Output::i()->jsonLd['topic']['comment'][] = array(
					...
					'author'	=> array(
						'@type'		=> 'Person',
						'name'		=> htmlspecialchars( $comment->author_name ?: \IPS\Member::load( $comment->author_id )->name, ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE ),
						'url'		=> (string) \IPS\Member::load( $comment->author_id )->url(),
