...
class _Goal extends \IPS\Node\Model
{
	...
	protected static function recountDonationGoals()
	{
		...
		\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => $count ), array( 'conf_key=?', 'donation_goals' ) );
		\IPS\Settings::i()->donation_goals = $count;
	...
	public function url( $action=NULL )
	{
		...
		if( $this->_url === null )
		{
			$this->_url = \IPS\Http\Url::internal( 'app=nexus&module=clients&controller=donations&id=' . $this->_id, 'front', 'clientsdonate', array( $this->name_seo ), \IPS\Settings::i()->nexus_https );
		}
