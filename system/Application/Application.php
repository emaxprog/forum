...
class _Application extends \IPS\Node\Model
{
	...
	{		
		...
		foreach ( $correctMd5s as $file => $md5Hash )
		{
			...
			/* MD5 check */
			if ( !file_exists( \IPS\ROOT_PATH . $file ) or !\IPS\Login::compareHashes( md5( preg_replace( '#\s#', '', utf8_decode( file_get_contents( \IPS\ROOT_PATH . $file ) ) ) ), $md5Hash ) ) // We strip whitespace since FTP in ASCII mode will change the whitespace characters
			{
