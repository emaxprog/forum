...
trait Reactable
{
	...
	public function removeReaction( \IPS\Member $member = NULL )
	{
		...
		try
		{
			...
			{
				$idColumn	= static::$databaseColumnId;
				$rep		= \IPS\Db::i()->select( '*', 'core_reputation_index', array( "member_id=? AND rep_class=? AND type=? AND type_id=?", $member->member_id, get_class( $this ), static::reactionType(), $this->$idColumn ) )->first();
			}
	...
	{
		$idColumn = static::$databaseColumnId;
		$where = array( array( 'rep_class=? AND type=? AND type_id=? AND reaction_enabled=?', get_class( $this ), static::reactionType(), $this->$idColumn, 1 ) );
	...
	public function reacted( \IPS\Member $member = NULL )
	{
		...
		try
		{
			...
			{
				return \IPS\Content\Reaction::load( \IPS\Db::i()->select( 'reaction', 'core_reputation_index', array( "member_id=? AND rep_class=? AND type=? AND type_id=?", $member->member_id, get_class( $this ), static::reactionType(), $this->$idColumn ) )->first() );
			}
		}
		catch( \UnderflowException $e )
		{
	...
	public function reactBlurb()
	{
		if ( $this->reactBlurb === NULL )
		{
			...
			if ( count( $this->reactions() ) )
			{
				...
				foreach( \IPS\Db::i()->select( 'reaction', 'core_reputation_index', array( "app=? AND rep_class=? AND type=? AND type_id=?", $app, get_class( $this ), static::reactionType(), $this->$idColumn ) ) AS $rep )
				{
		...
		if( $this->likeBlurb === NULL )
		{
			...
			if ( $numberOfLikes )
			{
				...
				else
				{
					...
					foreach ( \IPS\Db::i()->select( '*', 'core_reputation_index', array( 'app=? AND type=? AND type_id=? AND member_id!=?', $app, static::reactionType(), $this->$idColumn, ( \IPS\Member::loggedIn()->member_id ) ?: 0 ), 'RAND()', $userLiked ? 17 : 18 ) as $rep )
					{
						if ( $i < ( $userLiked ? 2 : 3 ) )
						{
