...
class _Facebook extends PromoteAbstract
{
	...
	public function getPhoto()
	{
		...
		{ 
			/* Fetch again */
			$response = \IPS\Http\Url::external( "https://graph.facebook.com/" . $this->settings['page'] . "/picture?type=large" )->request()->get();
	...
	public function canPostToPage()
	{
		try
		{
			$response = \IPS\Http\Url::external( "https://graph.facebook.com/{$this->member->fb_uid}/permissions" )->setQueryString( 'access_token', $this->member->fb_token )->request()->get()->decodeJson();
	...
	public function getPages()
	{
		...
		try
		{
			$response = \IPS\Http\Url::external( "https://graph.facebook.com/{$this->member->fb_uid}/accounts" )->setQueryString( 'access_token', $this->member->fb_token )->request()->get()->decodeJson();
	...
	public function post( $promote )
	{
		...
		$items = \IPS\Http\Url::external( "https://graph.facebook.com/" . $this->settings['page'] . "/feed" )->setQueryString( array( 'limit' => 20, 'access_token' => $this->settings['token'] ) )->request()->get()->decodeJson();
		...
		if ( ! $photos or count( $photos ) === 1 )
		{
			...
			try
			{
				if ( $photos )
				{
					...
					$this->response = \IPS\Http\Url::external( "https://graph.facebook.com/" . $this->settings['page'] . "/photos" )->request()->post( array(
				...
				{
					$this->response = \IPS\Http\Url::external( "https://graph.facebook.com/" . $this->settings['page'] . "/feed" )->request()->post( array(
						'message' => $promote->text['facebook'],
						'access_token' =>  $this->settings['token'],
						'link' => $promote->short_link
					) )->decodeJson();
		...
		else
		{
			...
			try
			{
				$this->response = \IPS\Http\Url::external( "https://graph.facebook.com/" . $this->settings['page'] . "/albums" )->request()->post( array(
				...
				{
					$this->response = \IPS\Http\Url::external( "https://graph.facebook.com/" . $newAlbumId . "/photos" )->request()->post( array(
						'message' => $promote->text['facebook'],
