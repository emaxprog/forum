...
class _Twitter extends PromoteAbstract
{
	...
	protected function twitter()
	{
		...
		{
			static::$twitter = \IPS\Login::getHandler('Twitter');
		}
	...
	public function canPostToPage()
	{
		try
		{
			$response = $this->twitter()->sendRequest( 'get', 'https://api.twitter.com/1.1/account/verify_credentials.json', array(), $this->settings['token'], $this->settings['secret'] );
	...
	public function form( $text, $link=null, $content=null )
	{
		...
		if ( mb_strlen( $textToActuallyUse ) < 140 )
		{
			/* Got any tags to add? */
			if ( count( $this->settings['tags'] ) )
			{
				...
				else
				{
					foreach( $this->settings['tags'] as $tag )
					{
						...
						if ( mb_strlen( $textToActuallyUse . $considerAddingThis ) < 140 )
						{
		...
		$return[] = new \IPS\Helpers\Form\TextArea( 'promote_social_content_twitter', $textToActuallyUse, FALSE, array( 'maxLength' => 300, 'rows' => 3 ) );
	...
	public function post( $promote )
	{
		...
		if ( count( $photos ) )
		{
			try
			{
				...
				foreach( $photos as $photo )
				{
					...
					if ( $done < 4 )
					{
						$this->response = $this->twitter()->sendRequest( 'post', 'https://upload.twitter.com/1.1/media/upload.json', array( 'media' => $photo->contents() ), $this->settings['token'], $this->settings['secret'] )->decodeJson();
		...
		try
		{
			...
			$this->response = $this->twitter()->sendRequest( 'post', "https://api.twitter.com/1.1/statuses/update.json", $send, $this->settings['token'], $this->settings['secret'] )->decodeJson();
