		...
		if ( in_array( 'authy', $availableMethods ) )
		{
			...
			if ( count( $availableMethods ) > 1 )
			{
				...
				try
				{
					...
					if ( $userHasAuthyApp )
					{						
						return \IPS\Theme::i()->getTemplate( 'login', 'core', 'global' )->authyAuthenticate( 'authy', TRUE, isset( $_SESSION['authyAuthError'] ) ? $_SESSION['authyAuthError'] : 'authy_error', FALSE, $availableMethods, $this->_onetouchInit( $member ), $url );
					}
				}
				catch ( \Exception $e ) { }
			}
